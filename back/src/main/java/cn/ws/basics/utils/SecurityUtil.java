package cn.ws.basics.utils;

import cn.ws.basics.exception.WsException;
...
import cn.ws.data.vo.PermissionDTO;
import cn.ws.data.vo.RoleDTO;
...
public class SecurityUtil {
    ...
    public String getToken(String username, Boolean saveLogin){
        if(WsNullUtils.isNull(username)){
            throw new WsException("username不能为空");
        }
        boolean saved = false;
        ...
        if(tokenProperties.getSaveRoleFlag()){
            for(PermissionDTO p : selectUser.getPermissions()){
                if(!WsNullUtils.isNull(p.getTitle()) && !WsNullUtils.isNull(p.getPath())) {
        ...
        if(tokenProperties.getSsoFlag()) {
            String oldToken = redisTemplate.get(WsLoginProperties.USER_TOKEN_PRE + selectUser.getUsername());
        ...
        // 保存至Redis备查
        if(saved){
            redisTemplate.set(WsLoginProperties.USER_TOKEN_PRE + selectUser.getUsername(), ansUserToken, tokenProperties.getUserSaveLoginTokenDays(), TimeUnit.DAYS);
            redisTemplate.set(WsLoginProperties.HTTP_TOKEN_PRE + ansUserToken, JSON.toJSONString(tokenUser), tokenProperties.getUserSaveLoginTokenDays(), TimeUnit.DAYS);
        }else{
            redisTemplate.set(WsLoginProperties.USER_TOKEN_PRE + selectUser.getUsername(), ansUserToken, tokenProperties.getUserTokenInvalidDays(), TimeUnit.MINUTES);
            redisTemplate.set(WsLoginProperties.HTTP_TOKEN_PRE + ansUserToken, JSON.toJSONString(tokenUser), tokenProperties.getUserTokenInvalidDays(), TimeUnit.MINUTES);
        }
        return ansUserToken;
    }
